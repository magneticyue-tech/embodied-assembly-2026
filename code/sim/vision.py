...
实现赛题三项视觉功能中的两项: 方块识别、托盘识别 (任务卡识别见 cognition.py)。
包含"整块刚性托盘板 + 遮挡"的处理逻辑。

托盘视觉策略:
- 托盘是一整块刚性矩形板, 板上 6 色槽布局相对板体固定。
- detect_board_pose(): 每步由板外轮廓矩形求板位姿(中心+角度)。
  外轮廓是整块板的大矩形, 不会被放上去的 30mm 方块遮挡。
- calibrate_slots(): 仅开局(无遮挡)执行一次, 标定"颜色 -> 板局部坐标"映射。
- 运行时取某色槽位置 = 板位姿 ⊕ 该色的板局部坐标 (slot_table_pos),
  不重新识别彩色中心, 因此放置后颜色被方块覆盖不影响定位。
...
# HSV 阈值表 (OpenCV: H 0-179)。真实赛场需在现场光源下逐色标定。
HSV_RANGES = {
...
def px_to_table_mm(u, v):
    """单应反投影: 像素 -> 台面 mm。仿真为正射投影; 真实赛场用标定得到的 H。"""
...
def detect_board_pose(img):
    """从板的外轮廓矩形求板位姿。返回 dict(x,y,deg,uv) 或 None。
    板是台面上面积最大的矩形灰块, 外轮廓不被 30mm 方块遮挡。"""
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # 板底色与台面灰度有差异, 用边缘+形态学找大矩形轮廓
    ...
    # 解析朝向: 判断检测到的 w 轴对应板的 W 还是 H, 据此修正 deg 的 90° 歧义
...
def calibrate_slots(img, board):
    """开局(无遮挡)执行一次: 识别每个彩色槽中心, 换算成板局部坐标并保存。
...
def detect_blocks(img, board=None, exclude_board_margin_mm=5):
    ...
    若给 board, 则排除落在板范围内的彩色块 (已放置方块或槽中心, 不属于待抓目标)。"""
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
...
# ============================================================
# OpenCVVision 实现 interfaces.VisionModule。每个方法直接委托到本模块的
# 同名全局函数。方法名与全局函数同名不互相遮蔽 (方法体内的裸名解析到
# module 全局作用域, 不是 self 属性, 故无递归)。
import interfaces


class OpenCVVision:
    """OpenCV 单目视觉管线, 实现 interfaces.VisionModule。

    仅作结构化封装: 算法在本模块的 module 级函数中, 本类只做委托,
    使 main.py 依赖抽象接口。真机实现使用同一接口, 更换标定后的参数。
    """
