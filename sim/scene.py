...
- 一整块刚性托盘板: 有自己的位姿(中心+旋转角), 板上 6 个色槽按固定布局排布,
  6 槽共享板的位姿 (随板一起平移/旋转)。
- 方块放上托盘后, 记录在 placed 里 -> 渲染时盖住该槽彩色中心 (模拟真实遮挡)。

perturb() 模拟"机器人前往拍照点期间裁判再次挪动": 自由方块会动, 托盘板整体也可能被挪动。
"""
import math
import random
import config as C
...
def _board_local_slots():
    """返回 6 个槽位在【板局部坐标】下的中心 (mm), 板中心为原点。
    顺序与 COLORS 对应位置无关, 仅给出 6 个固定锚点。"""
    pts = []
...
class Scene:
    ...
    def slot_table_pos(self, color):
        """某色槽当前台面坐标 = 板位姿 ⊕ 板局部锚点。中途板被挪动也实时正确。"""
    ...
    def place_block(self, color, x, y, deg):
        """方块放到托盘: 从待装配区移除, 记入 placed (渲染时盖住槽色)。"""
