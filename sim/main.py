...
def run_task2(scene, vis, cog, robot, evaluator, cam, io, slot_local):
    ...
    for item in parsed["sequence"]:
        ...
        img = cam.capture()
        board = vis.detect_board_pose(img)               # 重测板位姿(外轮廓, 遮挡鲁棒)
...
def main():
    ...
    # 依赖装配: 主流程只依赖抽象接口, 这里注入仿真实现 (真机换实现即可, 主流程不变)。
