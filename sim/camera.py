...
把 Scene (台面物理坐标 mm) 渲染成一张俯视 RGB 图像 (像素)。
真实赛场: 工业单目相机采集; 本仿真: 用 OpenCV 按 PX_PER_MM 正射投影绘制。
绘制顺序 (体现遮挡): 托盘板 -> 板上同心环色槽 -> 已放置方块(盖住槽色) -> 自由方块。
"""
import math
import numpy as np
import cv2
...
def render(scene, noise=True):
    ...
    # 1) 整块托盘板 (一个刚性矩形, 按板位姿旋转) —— 外轮廓永不被小方块遮挡
    bd = scene.board
